...
Sel_Col_Final_List = ['Id','Name','CurrencyIsoCode','RecordTypeId','Address_Line_1__c','Address_Line_6__c','Address_Line_7__c','Address_Line_8__c','Address_Line_9__c','City__c','Postcode__c']
...
def alter_final_df(Final_df):
    ...
    Final_df = Final_df.drop('AccountId','FirstName','LastName','Email','MailingCountryCode')
...
def Filter_Final_df(aggregate_final_df):
    Filter_Final_df = aggregate_final_df.filter((col("Address_Line_8__c") != 'NULL') & (col("Address_Line_1__c") != 'NULL'))
    
    Filter_Final_df = Filter_Final_df.sort(col("Src_Id").asc())
    
    return Filter_Final_df
    
def Handling_Insert_records(Filter_Final_df):
    Ins_records_df_index = Filter_Final_df.withColumn("idx",F.monotonically_increasing_id())
...
if __name__ == "__main__":
    ...
    Filter_Final_df = Filter_Final_df(aggregate_final_df)
    Final_Address_Contact_df = Handling_Insert_records(Filter_Final_df)
    Final_Address_Contact_select_df = Final_Address_Contact_df.select(*Sel_Col_Final_List)
    
    src_count = Contact_UKSUPDEV_Spark_df.count()
    Tgt_count = Final_Address_Contact_select_df.count()
