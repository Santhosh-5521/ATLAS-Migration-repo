...
def Recordtype_ID_ETL(AMS_Address_Spark_df,LKP_SF_Target_RecordType_Spark_df):
    ...
    v_Type_df = AMS_Address_Spark_df.withColumn('v_Type',when(instr(col('AMS_Address_Spark_df.Name'),'-Contact') > 0,'Contact')
                                                        .when(instr(col('AMS_Address_Spark_df.Name'),'-Account') > 0,'Account')            
    ...
    cond = [v_Type_df.sobject_type_in == LKP_SF_Target_RecordType_Spark_df.SobjectType, v_Type_df.v_Type == LKP_SF_Target_RecordType_Spark_df.Name]
...
def Market_Account__c_ETL(AMS_Address_Spark_df):
    ...
    Market_Account__c_df = v_Type_df.withColumn('Market_Account__c',when(v_Type_df.v_Type == 'Market Account',v_Type_df.Address_Line_8__c)
                                                 .otherwise('Null'))\
...
def Start_Date__c_ETL(AMS_Address_Spark_df):
    Start_date_Df = AMS_Address_Spark_df.withColumn('year', substring('CreatedDate', 1,4))\
                                        ...
                                        .withColumn('Start_Date__c',ltrim(rtrim(concat(col('year'),F.lit('-'),col('month'),F.lit('-'),col('day')))))\
                                        .select('Id','Start_Date__c')
    
    return Start_date_Df
    
def creating_final_df(AMS_Address_Spark_df,Recordtype_ID_df,Account__c_df,Contact__c_df,Market_Account__c_df,End_Date__c_df,Start_date_Df):
    AMS_Address_Spark_df = AMS_Address_Spark_df.alias('AMS_Address_Spark_df')
    ...
    Final_Df = AMS_Address_Spark_df.join(Recordtype_ID_df, col('AMS_Address_Spark_df.Id') == col('Recordtype_ID_df.Id'), 'left') \
                                        ...
                                        .select('AMS_Address_Spark_df.*','Recordtype_ID_df.RecordTypeId','Account__c_df.Account__c','Contact__c_df.Contact__c','Market_Account__c_df.Market_Account__c','End_Date__c_df.End_Date__c','Start_date_Df.Start_Date__c')
...
if __name__ == "__main__":
    ...
    Final_df = creating_final_df(AMS_Address_Spark_df,Recordtype_ID_df,Account__c_df,Contact__c_df,Market_Account__c_df,End_Date__c_df,Start_date_Df)
